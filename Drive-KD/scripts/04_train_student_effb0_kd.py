...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Drive-EffB0-BiFPN-KD student.")
    parser.add_argument(
        "--config",
        type=str,
...
def main() -> None:
    ...
    init_cfg = training.get("init_from_supervised_student", {})
    if bool(init_cfg.get("enabled", False)):
        init_ckpt = init_cfg["checkpoint"]
        print(f"[init student] {init_ckpt}")
        load_checkpoint(
            path=init_ckpt,
            model=model,
            map_location=device,
    ...
    criterion = MultiTaskKDLoss.from_config(
    ...
    for epoch in range(start_epoch, int(training["epochs"]) + 1):
        train_log = train_student_kd_one_epoch(
            model=model,
            loader=train_loader,
            optimizer=optimizer,
            scaler=scaler,
        ...
        val_log = validate_one_epoch(
            ...
            epoch=epoch,
            amp_enabled=amp_enabled,
            desc=f"student KD val epoch {epoch}",
        )
    ...
    print("[done] Student KD training complete.")
