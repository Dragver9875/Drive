...
def train_student_kd_one_epoch(
    *,
    ...
    pbar = tqdm(loader, desc=f"student KD train epoch {epoch}")
        ...
        pbar.set_postfix(
            {
                ...
                "sup": running.get("loss_supervised_weighted", 0.0) / max(1, n_batches),
                "prob": running.get("loss_probability_kd_weighted", 0.0) / max(1, n_batches),
                "bound": running.get("loss_boundary_kd_weighted", 0.0) / max(1, n_batches),
                "att": running.get("loss_attention_kd_weighted", 0.0) / max(1, n_batches),
            }
        )
